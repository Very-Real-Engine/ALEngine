...
#define NON_CURRENT_ANIMATION_BOOL false
	...
	void setRepeatAll(const std::vector<bool> repeats)
	{
		m_Repeats = repeats;
	};
	...
	std::vector<bool> getRepeatAll()
	{
		return m_Repeats;
	};
	std::vector<alglm::mat4> &getCurrentPose()
	{
	...
	std::shared_ptr<AnimationStateManager> getStateManager()
	{
  ...
  private:
	...
	std::vector<alglm::mat4> m_CurrentPose;
