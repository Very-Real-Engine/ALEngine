...
std::unique_ptr<RenderPass> RenderPass::createColliderRenderPass()
{
	std::unique_ptr<RenderPass> renderPass = std::unique_ptr<RenderPass>(new RenderPass());
	renderPass->initColliderRenderPass();
	return renderPass;
}

void RenderPass::initColliderRenderPass()
{
	auto &context = VulkanContext::getContext();
	VkDevice device = context.getDevice();
	...
	if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS)
	{
		throw std::runtime_error("failed to create collider render pass!");
	}
}
